...
# Alterations and customizations by Elliott Kember <http://riothq.com>
...
if __FILE__ == $0
  ...
  if ARGV[0] == "server" && ARGV.size == 3
    ...
    # This checks whether our parent process has died!
    Thread.new do
      while true
    ...
    server = WebSocketServer.new(:accepted_domains => ARGV[1].split(","), :port => ARGV[2].to_i())
      
    # puts("Server is running at port %d" % server.port)
      
    # Let's trap the INT interruption signal and reload CSS when we get it. 
    # This is Elliott's clever trick for sending messages to Ruby processes.
    # It's not ideal, but it works.

    trap("INT") { 
      @websockets.each do |websocket|
        websocket.send('css') rescue true
      end
    }
